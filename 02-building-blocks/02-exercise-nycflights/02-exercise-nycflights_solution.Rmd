...
plot(var1$date, var1$arr_delay, main="Scatterplot",
   xlab="Date ", ylab="Arrical Delay ", pch=19) 
...
a<-dplyr::inner_join(flights, planes, by = c( "tailnum"))
b<-dplyr::inner_join(a, weather, by = c("origin", "time_hour"))

names(airports)[1] <- "origin"

c<-dplyr::inner_join(a, airports, by = c("origin"))
